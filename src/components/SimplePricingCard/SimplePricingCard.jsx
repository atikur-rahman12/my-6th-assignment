    ...
    <div>
      <div className="card w-96 bg-base-100 shadow-sm h-full mx-auto mt-10 transform transition-transform duration-300 hover:-translate-y-2 hover:shadow-lg">
        <div className={`card-body border rounded-2xl ${
          ...
          <h2 className={`text-3xl font-bold text-black ${popular ? "text-white/80" : "text-black"}`}>{name}</h2>
          <p className={`${popular ? "text-white/80" : "text-gray-500"}`}>
            {description}
          </p>
          <span className="text-xl mt-2">
            <span className={`text-4xl font-bold ${popular ? "text-white/80" : "text-black"}`}>{price}</span>{" "}
          ...
          <ul className="mt-6 flex flex-col gap-2 text-xs">
            {features.map((feature, index) => (
              <li key={index}>
                ...
                <span className={`${popular ? "text-white/80" : "text-gray-500"}`}>{feature}</span>
              </li>
            ))}
          </ul>

          <div className="mt-6">
            <button className={`btn btn-block border-none font-bold rounded-full ${
                popular ? "bg-white text-indigo-600" : "bg-linear-to-r from-indigo-600 to-purple-500"}`}>
                    {button}
            </button>
          </div>
        </div>
